...
All code for image processing is in `./Program/Controllers/preProcessingController.py`.
...
All code for the simple driver is in `./Program/Controllers/simplecontroller.py`.
...
All physical IO for the vehicle is handled by the `gpio` module, found in `/Program/IO/gpio.py`. Submodules of `gpio` - `carDrive`, `camera`, and `imu` -  can be imported separately, but are included in `gpio` for ease of use.

### IO

`gpio` is a wrapper containing `carDrive`, `camera`, and `imu` submodules, as well as indicator LED controls.

| Property                | Description                                                                                                                                           | Value    |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| `GPIO`                  | The `GPIO` object used to control the vehicle.                                                                                                        | `GPIO`   |
| `carDrive`                 | The [`carDrive`](#carDrive) submodule.                                                                                                                      | `carDrive`  |
...
| `stop()`                  | Stops the `carDrive` module. Sets throttle to 0. `steer(str)` has no effect after calling this.                                                                                           | Nothing                         |

### Camera

`camera` reads inputs from the cameras, writes images to file, and can stream to the Control Panel via the server in `Util`. It automatically starts video streams from two cameras using `nvcam` when imported.
...
The driving decisions are handled by the drivers in `/Program/Controller/`. There are three controllers - `simplecontroller`, `slamcontroller`, and `borkencontroller` - though two (`slamcontroller` and `borkencontroller`) are not functional. Images are first processed by the `preProcessingController` module to generate location data used in the controllers.

### preProcessingController

`preProcessingController` is the image processor that is used to extract data from images. It mainly handles undistorting, filtering, and running detection algorithms on images. *Note: Images are 3-dimensional `numpy.ndarray`, where (0, 0) is the top left corner.*
